...
static std::vector<fs::path> list_images(const fs::path& dir, const std::string& prefix, const std::string& ext = ".jpg"){
    ...
    for (const auto& entry : fs::directory_iterator(dir)) {
        ...
        if (!ends_with_case_insensitive(name, ext)){ 
            continue;
        }
        files.push_back(p);
    }
...
// ASM(Blur + Sobel)
extern "C" void blur3x3(const uint8_t* in, uint8_t* out, int width, int height);
extern "C" void conv3x3_u8_s16_asm(const uint8_t* in, int16_t* out, int w, int h, const int* k);
...
void print_metrics(const Metrics& m, double elapsed_ms) {
...
int main(int argc, char** argv){
    ...
    if (moon_files.empty() && no_moon_files.empty()) {
        ...
        std::cout << "Expected patterns:\n";
        std::cout << "  " << moon_dir.string() << "/moon*.jpg\n";
        std::cout << "  " << no_moon_dir.string() << "/no_moon*.jpg\n";
        return 0;
    }
    ...
    for (const auto& p : moon_files) {
        std::string out_png;            // فقط وقتی ماه پیدا شد استفاده میشه
        
        out_png = (out_dir / (stem_no_ext(p.filename().string()) + ".png")).string();
